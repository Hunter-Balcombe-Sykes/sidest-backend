...
/**
 * Every 5 minutes: send end notifications for promotions that just expired.
 */
class SendPromotionEndNotificationsJob implements ShouldQueue
{
    ...
    public function handle(NotificationPublisher $notificationPublisher): void
    {
        BrandPromotion::query()
            ...
            ->whereNull('end_notification_sent_at')
            ->orderBy('id')
    ...
    {
        $lockName = 'promotion-end-notify:' . $promotionId;
        ...
        try {
            $promotion = DB::transaction(function () use ($promotionId): ?BrandPromotion {
                ...
                if (
                    ! $promotion->is_active
                    || $promotion->end_notification_sent_at !== null
            ...
            BrandPromotion::query()
                ->where('id', $promotionId)
                ->whereNull('end_notification_sent_at')
                ->update(['end_notification_sent_at' => now()]);
        } catch (Throwable $e) {
            Log::error('SendPromotionEndNotificationsJob: failed to send notifications', [
    ...
    private function notifyAffiliates(BrandPromotion $promotion, NotificationPublisher $publisher): void
    {
        ...
        $body = "A promotion has ended: {$promotion->name}";
        ...
        $notify = function (string $affiliateId) use ($publisher, $promotionId, $title, $body): void {
            try {
                $publisher->publish(
                    ...
                    title: $title,
                    body: $body,
                    dedupeKey: "promotion.ended.{$promotionId}.{$affiliateId}",
                    ctaUrl: '/account/store',
                );
            } catch (Throwable $e) {
                Log::warning('SendPromotionEndNotificationsJob: failed to notify affiliate', [
