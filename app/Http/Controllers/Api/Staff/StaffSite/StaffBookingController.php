...
// Staff inspector for a brand's booking settings + smart-mode analytics (#BOOK-1).
// Booking settings are read out of site.settings (the brand-side PATCH writes there
// via ProfessionalSiteController::updateBookingSettings — there's no brand GET, so
// this exposes the same shape from the storage location). Analytics mirrors
// BookingAnalyticsController::myOverview and shares its cache key so a single bust
// covers both endpoints.
class StaffBookingController extends ApiController
{
    ...
    /**
     * GET /staff/professionals/{professional}/booking/settings
     * Returns booking_mode + manual_booking_url stored in site.settings.
     */
    public function settings(Professional $professional): JsonResponse
    {
        $site = Site::query()
        ...
        return $this->success([
            ...
            'services_auto_sync_enabled' => (bool) ($settings['services_auto_sync_enabled'] ?? false),
        ]);
    }

    /**
     * GET /staff/professionals/{professional}/booking/analytics
     * Mirrors BookingAnalyticsController::myOverview. Same cache key, same shape.
     */
    public function analytics(Request $request, Professional $professional): JsonResponse
    {
    ...
    /**
     * @return array{from: string, to: string, group_by: string, explicit_range: bool}
     */
    ...
    /**
     * @param  array{from: string, to: string, group_by: string, explicit_range: bool}  $filters
     * @return array{from: mixed, to: mixed, range_from: string, range_to: string, group_by: string, granularity: string, bucket_timezone: string|null, use_hourly: bool}
     */
