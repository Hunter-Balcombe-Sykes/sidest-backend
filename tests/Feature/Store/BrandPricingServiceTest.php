...
it('resolves effective commission with override fallback', function () {
    ...
    expect($service->effectiveCommissionRate(22.4, 15.0, 11.0))->toBe(22.4);
    expect($service->effectiveCommissionRate(null, 17.5, 11.0))->toBe(17.5);
    expect($service->effectiveCommissionRate(null, null, 12.5))->toBe(12.5);
    expect($service->effectiveCommissionRate(null, null, null))->toBe((float) config('comet.store.default_commission_rate', 15));
});
